...
| char_cost_dict | dict | [CHAR_COSTS](src/delfuzz/costs.py) | Dictionary of custom character-level costs. |
| token_cost_dict | dict | [TOKEN_COSTS](src/delfuzz/costs.py) | Dictionary of custom token-level costs. |
| placeholders | list[tuple[str, str]] | [MULTIGRAPH_PLACEHOLDERS](src/delfuzz/costs.py) | List of multigraph-to-placeholder mappings used to treat common Spanish multigraphs as their own singular characters. |
...
`score` accepts custom cost dictionaries, allowing you to modify or replace the defaults ([CHAR_COSTS](src/delfuzz/costs.py) and/or [TOKEN_COSTS](src/delfuzz/costs.py)).
